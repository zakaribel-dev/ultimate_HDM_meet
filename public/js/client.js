...
// misc
let swBg = '#01619E'; // swAlert background color
...
async function whoAreYou() {
    ...
    Swal.fire({
        allowOutsideClick: false,
        allowEscapeKey: false,
        title: '<a href="/" style="text-decoration: none;"><img src="../images/little_logo_blue.svg"></a>',
        position: 'center',
        input: 'text',
        ...
        html: initUser,
        confirmButtonText: `Rejoindre conférence`,
        customClass: { popup: 'init-modal-size' },
        showClass: { popup: 'animate__animated animate__fadeInDown' },
        hideClass: { popup: 'animate__animated animate__fadeOutUp' },
        inputValidator: async (value) => {
            if (!value) return 'Entrez un nom svp..';
            // Long name
            if (value.length > 30) return 'Votre nom doit faire 30 caractères max!';
            ...
            if (await checkUserName()) {
                return 'Nom déjà présent dans la conférence ..!';
            } else {
...
async function loadRemoteMediaStream(stream, peers, peer_id, kind) {
    ...
    switch (kind) {
        case 'video':
            ...
            // no mobile devices
            if (!isMobileDevice) {
                setTippy(remotePeerName, 'Nom du participant', 'bottom');
                setTippy(remoteHandStatusIcon, 'La main du participant est levée', 'bottom');
                setTippy(remoteVideoStatusIcon, 'La vidéo du participant est activée', 'bottom');
                setTippy(remoteAudioStatusIcon, 'L\'audio du participant est activé', 'bottom');
                setTippy(remoteAudioVolume, '🔊 Volume', 'top');
                setTippy(remoteVideoAudioUrlBtn, 'Envoyer vidéo ou audio', 'bottom');
                setTippy(remotePrivateMsgBtn, 'Envoyer un message privé', 'bottom');
                setTippy(remoteFileShareBtn, 'Envoyer un fichier', 'bottom');
                setTippy(remoteVideoToImgBtn, 'Prendre une capture d\'écran', 'bottom');
                setTippy(remotePeerKickOut, 'Exclure', 'bottom');
                setTippy(remoteVideoFullScreenBtn, 'Mode plein écran', 'bottom');
                setTippy(remoteVideoZoomInBtn, 'Zoom avant dans la vidéo', 'bottom');
                setTippy(remoteVideoZoomOutBtn, 'Zoom arrière dans la vidéo', 'bottom');
                setTippy(remoteVideoPiPBtn, 'Basculer en mode image dans l\'image', 'bottom');
                setTippy(remoteVideoPinBtn, 'Basculer en mode vidéo épinglée', 'bottom');
                setTippy(remoteVideoMirrorBtn, 'Basculer en mode miroir pour la vidéo', 'bottom');
...
function handleVideoPlayerFs(videoId, videoFullScreenBtnId, peer_id = null) {
    ...
    videoFullScreenBtn.addEventListener('click', (e) => {
        if (videoPlayer.classList.contains('videoCircle')) {
            return userLog('toast', 'Plein écran non autorisé lorsque la vidéo est en mode confidentialité');
        }
        gotoFS();
    });
    ...
    function showMsg() {
        userLog('toast', 'Le mode plein écran fonctionne lorsque la vidéo est activée.');
    }
...
function handleFileDragAndDrop(elemId, peer_id, itsMe = false) {
    ...
    videoPeer.addEventListener('drop', function (e) {
        e.preventDefault();
        if (itsMe) {
            return userLog('warning', 'Vous ne pouvez pas vous envoyer des fichiers à vous-même');
        }
        if (sendInProgress) {
            return userLog('warning', 'Veuillez patienter que le fichier précédent soit envoyé');
        }
        if (e.dataTransfer.items && e.dataTransfer.items.length > 1) {
            return userLog('warning', 'Veuillez faire glisser et déposer un seul fichier.');
        }
        ...
        if (e.dataTransfer.items) {
            ...
            if (item.isDirectory) {
                return userLog('warning', 'Veuillez faire glisser et déposer un seul fichier, pas un dossier', 'top-end');
            }
        ...
        btnPn.addEventListener('click', () => {
            ...
            if (isVideoPinned) {
                ...
                if (pinnedVideoPlayerId != videoPlayer.id) {
                    ...
                    return userLog('toast', 'Une autre vidéo semble épinglée. Désépinglez-la avant d\'épingler celle-ci', 5000);
                }
...
function handleVideoZoomInOut(zoomInBtnId, zoomOutBtnId, mediaId, peerId = null) {
    ...
    if (buttons.local.showZoomInOutBtn) {
        zoomIn.addEventListener('click', () => {
            if (isVideoOf(id)) return userLog('toast', 'Le zoom fonctionne lorsque la vidéo est activée.');
            if (isVideoPrivacyMode(video)) return userLog('toast', 'Le zoom n\'est pas autorisé lorsque la vidéo est en mode confidentialité');
            zoom = zoom + 0.1;
            setTransform();
        });

        zoomOut.addEventListener('click', () => {
            if (isVideoOf(id)) return userLog('toast', 'Le dézoom fonctionne lorsque la vidéo est activée.');
            if (isVideoPrivacyMode(video)) return userLog('toast', 'Le dézoom n\'est pas autorisé lorsque la vidéo est en mode confidentialité');
            zoom = zoom - 0.1;
            setTransform();
        });
    }
...
function handleVideoToImg(videoStream, videoToImgBtn, peer_id = null) {
    ...
    videoBtn.addEventListener('click', () => {
        if (video.classList.contains('videoCircle')) {
            return userLog('toast', 'Capture d\'écran non autorisé en mode privé');
        }
        if (peer_id !== null) {
...
function setChatRoomBtn() {
    ...
    msgerCPBtn.addEventListener('click', (e) => {
        if (!thereArePeerConnections()) {
            return userLog('info', 'Pas de participant détecté');
        }
    ...
    msgerShowChatOnMsg.addEventListener('change', (e) => {
        ...
        showChatOnMessage
            ? msgPopup('info', 'Le chat sera affiché lorsque vous recevrez un nouveau message', 'top-end', 3000)
            : msgPopup('info', 'Le chat ne sera pas affiché lorsque vous recevrez un nouveau message', 'top-end', 3000);
    ...
    if (isSpeechSynthesisSupported) {
        msgerSpeechMsg.addEventListener('change', (e) => {
            ...
            speechInMessages
                ? msgPopup('info', 'Lorsque vous recevez un nouveau message, il sera converti en discours.', 'top-end', 3000)
                : msgPopup('info', 'Vous avez désactivé les messages vocaux.', 'top-end', 3000);
...
function setCaptionRoomBtn() {
    if (speechRecognition && buttons.main.showCaptionRoomBtn) {
        ...
        captionClean.addEventListener('click', (e) => {
            ...
            userLog('info', 'Il n\'y a pas de sous-titres à supprimer');
        });
        ...
        captionSaveBtn.addEventListener('click', (e) => {
            ...
            userLog('info', 'Il n\'y a pas de sous-titres à sauvegarder');
        });
...
function setMySettingsBtn() {
    ...
    switchSounds.addEventListener('change', (e) => {
        ...
        lS.setSettings(lsSettings);
        userLog('toast', `${icons.sounds} Notifier et sons ` + (notifyBySound ? 'ON' : 'OFF'));
        playSound('switch');
    });
    switchShare.addEventListener('change', (e) => {
        ...
        lS.setSettings(lsSettings);
        userLog('toast', `${icons.share} Partager la conférence à rejoindre ` + (notify ? 'ON' : 'OFF'));
        playSound('switch');
    });
    ...
    if (isMobileDevice) {
        ...
        switchPushToTalk.addEventListener('change', (e) => {
            ...
            userLog('toast', `👆 Appuyez pour parler ` + (isPushToTalkActive ? 'ON' : 'OFF'));
            playSound('switch');
        });
    }
    ...
    switchAudioPitchBar.addEventListener('change', (e) => {
        ...
        lS.setSettings(lsSettings);
        userLog('toast', `${icons.pitchBar} Barre de hauteur de tonalité audio ` + (isAudioPitchBar ? 'ON' : 'OFF'));
        playSound('switch');
    });
    ...
    switchH264Recording.addEventListener('change', (e) => {
        ...
        lS.setSettings(lsSettings);
        userLog('toast', `${icons.codecs} Prioriser l'enregistrement en h.264 ` + (recPrioritizeH264 ? 'ON' : 'OFF'));
        playSound('switch');
    });
    ...
    themeCustom.check.onchange = (e) => {
        ...
        lS.setSettings(lsSettings);
        setTheme();
        userLog('toast', `${icons.theme} Thème personnalisé conservé` + (themeCustom.keep ? 'ON' : 'OFF'));
        playSound('switch');
        e.target.blur();
    };
}
...
function hideShowMySettings() {
    if (!isMySettingsVisible) {
        ...
        setTippy(mySettingsBtn, 'Fermer les paramètres', placement);
...
async function updateMyPeerName() {
    ...
    if (isHtml(myPeerNameSet.value)) {
        myPeerNameSet.value = '';
        return userLog('warning', 'Nom invalide !');
    }
    ...
    setPeerAvatarImgName('myProfileAvatar', myPeerName);
    setPeerChatAvatarImgName('right', myPeerName);
    userLog('toast', 'Mon nom est changé en ' + myPeerName);
}
...
function setMyHandStatus() {
    ...
    if (myHandStatus) {
        ...
        elemDisplay(myHandStatusIcon, false);
        setTippy(myHandBtn, 'Baisser la main', placement);
    }
...
function setMyAudioStatus(status) {
    ...
    setTippy(myAudioStatusIcon, status ? 'Mon audio est activé' : 'on audio est désactivé', 'bottom');
    setTippy(audioBtn, status ? 'Arrêter l\'audio' : 'Commencer l\'audio', placement);
...
function setMyVideoStatus(status) {
    ...
    if (!isMobileDevice) {
        if (myVideoStatusIcon) setTippy(myVideoStatusIcon, status ? 'Ma vidéo est activée' : 'Ma vidéo est désactivée', 'bottom');
        setTippy(videoBtn, status ? 'Arrêter la vidéo' : 'Commencer la vidéo', placement);
    }
...
function setPeerAudioStatus(peer_id, status) {
    ...
    if (peerAudioStatus) {
        ...
        setTippy(peerAudioStatus, status ? 'L\'audio du participant est activé' : 'L\'audio du participant est désactivé', 'bottom');
        status ? playSound('on') : playSound('off');
    }
    if (peerAudioVolume) {
    ...
    peerAudioBtn.onclick = () => {
        if (peerAudioBtn.className === className.audioOn) {
            isPresenter
                ...
                : msgPopup('warning', 'Seul le/la présentateur/trice peut désasctiver le son d\'un utilisateur', 'top-end', 4000);
        }
    };
}
    ...
    peerVideoBtn.onclick = () => {
        if (peerVideoBtn.className === className.videoOn) {
            isPresenter
                ...
                : msgPopup('warning', 'Seul le/la présentateur/trice peut désasctiver la vidéo d\'un utilisateur', 'top-end', 4000);
        }
    };
}
    ...
    Swal.fire({
        ...
        if (result.value) {
            ...
            appendMessage(
                ...
                rightChatAvatar,
                'right',
                pMsg + '<br/><hr>Message privé à ' + toPeerName,
                true,
            );
            userLog('toast', 'Message envoyé à' + toPeerName + ' 👍');
        }
    });
}
...
function disableAllPeers(element) {
    if (!thereArePeerConnections()) {
        return userLog('info', 'Pas de participant(s) détecté(s)');
    }
    Swal.fire({
        background: "white",
        position: 'center',
        imageUrl: element == 'audio' ? images.audioOff : images.videoOff,
        title: element == 'audio' ? 'Couper le son de tout le monde sauf vous?' : 'Cacher la caméra de tout le monde sauf vous ?',
        text:
            element == 'audio'
                ? "Une fois leurs son coupé, vous ne pourrez pas les rendre audibles, mais ils peuvent se rendre audibles à tout moment."
                : "Une fois leur caméra cachée, vous ne pourrez pas les rendre visibles, mais ils peuvent se rendre visibles à tout moment.",
        showDenyButton: true,
        confirmButtonText: element == 'audio' ? `Couper` : `Cacher`,
        ...
        if (result.isConfirmed) {
            switch (element) {
                case 'audio':
                    userLog('toast', 'Tout le monde est muet 👍');
                    emitPeersAction('muteAudio');
                    break;
                case 'video':
                    userLog('toast', 'Tout le monde est caché 👍');
...
function ejectEveryone() {
    if (!thereArePeerConnections()) {
        return userLog('info', 'Pas de participants détecté');
    }
    Swal.fire({
        background: 'white',
        imageUrl: images.leave,
        position: 'center',
        title: 'Virer tout le monde sauf vous ?',
        text: 'êtes vous sur de vouloir virer tout le monde sauf vous ?',
        showDenyButton: true,
        confirmButtonText: `Oui`,
        denyButtonText: `Non`,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
...
function disablePeer(peer_id, element) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            switch (element) {
                case 'audio':
                    userLog('toast', 'Sons bien coupés👍');
                    emitPeerAction(peer_id, 'muteAudio');
                    break;
                case 'video':
                    userLog('toast', 'Videos bien cachées 👍');
        ...
        switch (action) {
            case 'lock':
                ...
                Swal.fire({
                    ...
                    input: 'text',
                    inputPlaceholder: 'Mettre un mot de passe à la conférence',
                    confirmButtonText: `OK`,
                    denyButtonText: `Annuler`,
                    showClass: { popup: 'animate__animated animate__fadeInDown' },
                    hideClass: { popup: 'animate__animated animate__fadeOutUp' },
...
function handleUnlockTheRoom() {
    ...
    Swal.fire({
        ...
        background: 'white',
        imageUrl: images.locked,
        title: 'Oops, la conférence est verrouillé',
        input: 'text',
        inputPlaceholder: 'Entrez le mot de passe de la conférence',
        confirmButtonText: `OK`,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
        hideClass: { popup: 'animate__animated animate__fadeOutUp' },
        inputValidator: (pwd) => {
            if (!pwd) return 'Entrez le mot de passe de la conférence svp';
            thisRoomPassword = pwd;
        },
    }).then(() => {
        const config = {
    ...
    switch (type) {
        ...
        case 'pdfFile':
            Swal.fire({
                ...
                background: 'white',
                position: 'center',
                title: 'Selectionnez le PDF',
                input: 'file',
                inputAttributes: {
                    accept: wbPdfInput,
                    'aria-label': 'Selectionnez le PDF',
                },
                showDenyButton: true,
                confirmButtonText: `OK`,
                denyButtonText: `Annuler`,
...
function confirmCleanBoard() {
    ...
    Swal.fire({
        ...
        imageUrl: images.delete,
        position: 'center',
        title: 'Nettoyer le tableau',
        text: 'êtes vous sûr de vouloir nettoyer le tableau ?',
        showDenyButton: true,
        confirmButtonText: `Oui`,
        denyButtonText: `Non`,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
...
function sendFileInformations(file, peer_id, broadcast = false) {
    ...
    if (fileToSend && fileToSend.size > 0) {
        ...
        if (isHtml(fileToSend.name) || !isValidFileName(fileToSend.name))
            return userLog('warning', 'Nom de fichier invalide !');
        ...
        userLog('error', 'Fichier non valide ou vide.');
    }
}
    ...
    if (isImageURL(incomingFileInfo.file.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                imageAlt: 'HDM telechargement image',
                showDenyButton: true,
...
function sendVideoUrl(peer_id = null) {
    ...
    Swal.fire({
        ...
        title: 'Partager video ou audio',
        text: 'Collez une URL de vidéo ou audio',
        input: 'text',
        showCancelButton: true,
        ...
        if (result.value) {
            result.value = filterXSS(result.value);
            if (!thereArePeerConnections()) {
                return userLog('info', 'Pas de participant(s) détécté(s)');
            }
            ...
            */
            if (!isVideoTypeSupported(result.value)) {
                return userLog('warning', 'Essayez une autre url..');
            }
...
function handlePeerKickOutBtn(peer_id) {
    ...
    peerKickOutBtn.addEventListener('click', (e) => {
        isPresenter
            ? kickOut(peer_id)
            : msgPopup('warning', 'Seul le présentateur peut éjecter le(s) participant(s)', 'top-end', 4000);
    });
}
